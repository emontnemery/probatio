...
- A `dict` or `list` subclass keeps the state it carries, not only its class.
  ...
  `__slots__` is dropped. Probatio copies it across through the standard
  `object.__getstate__` protocol (ADR-018), so a Home Assistant `NodeDictClass`
  still knows the file and line it came from after validation and a config error
  can still point at it. Additive: a plain `dict` or `list` has no such state.
